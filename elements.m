...
RootElements := function( G : Transpose := false ) 
    ...
    /* returns a generator for the center of the corresponding classical
       group. */
...
DiagonalElement := function( type, dim, q )
    ...
    if type eq "SL" then
        ...
        sc := (q^2-1)/GCD(q+1,dim);  
        zmodq1 := quo< Integers() | q^2-1 >;
        exp := Integers()!Solution( zmodq1!(q+2), zmodq1!sc ); 
        progy0 := (B.4*B.10*(B.10^-1)^B.2)^exp;  
        progy := One( B );
        
        for i in [1..(dim-2)/2] do
            progy1 := (progy0^(B.2^(i-1)))^i;
            progy := progy*progy1;
        end for;
        
        exp1 := Integers()!Solution( zmodq1!(q+1), 
                        zmodq1!(sc+Round(sc*((dim-2)/2))));
        return progy*(B.10^(B.2^-1))^exp1;
        
    elif type eq "SU" and IsOdd( dim ) then
        
        progZ := One( B );
        exp := Round((q^2-1)/GCD(dim,q+1));
        progZ1 := B.4^exp;
    ...
    elif type eq "Omega+" and dim mod 4 eq 0 then
        
        progZ := One( B );
        progZ1 := B.4^Round((q-1)/2);
        V := B.2;
        for i in [1..(dim)/4] do
            progZ := progZ*progZ1;
            progZ1 := progZ1^V^2;
        ...
        progZ1 := B.4^Round((q-1)/2);
        V := B.2;
        for i in [1..(dim)/2-1] do
            progZ := progZ*progZ1;
            progZ1 := progZ1^V;
        end for;
      
        exp1 := Round((q-1)*((q-3)/4-(dim-6)/4));
        return B.4^exp1*progZ;

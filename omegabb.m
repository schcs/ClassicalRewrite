...
// iconj(x, y) = (x^y)^-1 = (y^-1*x*y)^-1 = (y^-1*x^-1*y) = (x*y)^-1*y
iconj := func<x, y | (x*y)^-1*y>;
...
IsInH := function( G, g )
    ...
    //"conj test";
    for i in roots do
        //if (r1^g, i) ne One( G ) or (r2^g,i) ne One( G ) then return false; end if;
        //t1 := r1^g;
        t1 := gi*r1*g;
	if t1*i ne i*t1 then
            return false;
        end if;
	//t2 := r2^g;
	t2 := gi*r2*g;
	if t2*i ne i*t2 then
            return false;
        end if;
    end for;
...
b^-1*a*b = a
a*b = b*a
[Now b = a^g1 = g1^-1*a*g1]
a*g1^-1*a*g1 = g1^-1*a*g1*a
g1*a*g1^-1*a*y = a*g1*a
*/

// a^(a^e) = a <=> a*b = b*a, where b = a^e
dconj_test := func<a, e | a*b eq b*a where b:=a^e>;

/*
assert dconj_test(roots[i-1], g1)
    eq
(roots[i-1]^(roots[i-1]^g1) eq roots[i-1]);
*/

            //if roots[i-1]^(roots[i-1]^g1) eq roots[i-1] then 
            if dconj_test(roots[i-1], g1) then
                j := exp;
                found := "X";
                break;
            end if;
            ...
            if type eq "Omega" then
                ...
                if dconj_test(roots[i-1], g2) then
                    j := exp;
                    found := "XX";
                    break;
                end if;
       ...
       //D0 := D0^V1;
       //assert V1I*D0*V1 eq D0^V1;

       D0 := V1I*D0*V1;
       //OX11:=X11; OX12 := X12;
       X11 := iconj(X11, V1); X12 := iconj(X12, V1);
       //"iconj test"; assert X11 eq (OX11^V1)^-1; assert X12 eq (OX12^V1)^-1;
       X := X11; XX := X12;
   end for;
   ...
   //X22 := (X22^V1^-1)^(-1)^exp;
   //OX22 := X22;
   X22 := (V1*X22*V1I)^(-1)^exp;
   //assert X22 eq (OX22^V1^-1)^(-1)^exp;
   ...
   for i in [ case< type | "Omega-": dim/2, default: Floor( dim/2 )+1>..
       ...
       //OD0 := D0;
       D0 := V1*D0*V1I;
       //assert D0 eq OD0^V1^-1;

       //"iconj test"; assert X11 eq (OX11^V1)^-1; assert X12 eq (OX12^V1)^-1;

       //OX21 := X21; OX22 := X22;
       //X21 := (X21^V1^-1)^-1; X22 := (X22^V1^-1)^-1;
       X21 := iconj(X21, V1I); X22 := iconj(X22, V1I);
       //assert X21 eq (OX21^V1^-1)^-1; assert X22 eq (OX22^V1^-1)^-1;
...
w := QMeet( G, g : Transpose := true );
    if w cmpeq false then return false, _, _, _; end if;
    w := w[1];

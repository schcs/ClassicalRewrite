...
procedure ClassicalElementToSLPSpSUDim4( G, g, ~slp_left, 
          ~slp_right )
...
/* LEMMA 3.4(ii)  of Brooksbank's classical recognition paper 
   (JSC, vol 35, 2003)
   The input is:
       (1) b, q
   The output is:
  ...
  This is taken from Brooksbank's code
*/

ElementHavingPrescribedNorm := function( b, q )
...
procedure ClassicalElementToSLPSUDim3( G, g, ~slp_left, ~slp_right )
    ...
    /* Now g is diagonal of the form diag(lambda,lambda^-q,lambda^(q-1)). 
      The following procedure follows the procedure described on 
      page 235 of Brooksbank's "Constructive recognition of classical
      groups in their natural representations", JSC, vol 35, 2003, 195-239. */
      
    a := case< IsOdd( q ) | true:z^Round((q0+1)/2), false: 1, default: 1 >;
    delta := a^q0;
    lambda :=  g[1,1];
    gamma := lambda*delta;
    
    // find eta such that eta^(q0+1) eq -gamma-gamma^q0;
    eta := ElementHavingPrescribedNorm( -gamma-gamma^q0, q0 );
    
    vec := [ 0 : x in [1..d-3]] cat 
           [-a^2*gamma^(-q0),-a*(eta*gamma^-1)^q0];
    u1 := SiegelTransformToSLP( G, vec )^B.1;
    
    vec := [ 0 : x in [1..d-3]] cat [ gamma, eta^q0 ];
    u2 := SiegelTransformToSLP( G, vec );
    
    vec := [ 0 : x in [1..d-3]] cat 
           [ -a^2*gamma^-q0, -a*(eta*gamma^-q0)^q0]; 
    u3 := SiegelTransformToSLP( G, vec )^B.1;
